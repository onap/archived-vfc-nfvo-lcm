    ...
    def do_update(self):
        if self.update_type == "OPERATE_VNF":
            ...
            if not self.operate_vnf_data:
                logger.error('OperateVnfData does not exist or value is incorrect.')
            ...
            for vnf_update_data in self.operate_vnf_data:
                ...
                if status is JOB_MODEL_STATUS.FINISHED:
                    logger.info('nf[%s] update handle end' % vnf_update_params.get('vnfInstanceId'))
                    self.update_job(90,
                                    desc='nf[%s] update handle end' % vnf_update_params.get('vnfInstanceId'))
                else:
