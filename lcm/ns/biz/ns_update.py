...
class NSUpdateService(threading.Thread):
    ...
    def get_and_check_params(self):
        ...
        if not self.update_type:
            logger.error('UpdateType parameter does not exist or value is incorrect.')
            raise NSLCMException('UpdateType parameter does not exist or value incorrect.')

    def do_update(self):
        if self.update_type == "OPERATE_VNF":
            self.operate_vnf_data = ignore_case_get(self.request_data, "operateVnfData")
            ns_update_params = self.prepare_update_params(self.operate_vnf_data)
            status = self.do_ns_update(ns_update_params, 15)
            if status is JOB_MODEL_STATUS.FINISHED:
                logger.info('nf[%s] update handle end' % ns_update_params.get('nsInstanceId'))
                self.update_job(90,
    ...
    def do_ns_update(self, ns_update_params, progress):
        ...
        nf_service.start()
        self.update_job(progress, desc='nf[%s] update handle start' % ns_instance_id)
        ...
        if operational_states == "STARTED":
            ...
            if stop_type == "GRACEFUL":
                ...
                if not graceful_stop_timeout:
                    logger.error('gracefulStopTimeout does not exist or value is incorrect.')
                    raise NSLCMException(
            ...
            raise NSLCMException('OperationalStates should be started or stopped')
